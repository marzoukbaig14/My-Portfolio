...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: "Automated in/out line calling for padel. Overhead stadium rigs are expensive and see the court as a top-down diagram; LineGuard watches low and side-on, the angle a player actually sees the line from, which is exactly where the sport's one real dispute lives (the back-corner \"glass first or floor first?\"). Two cheap cameras off to the side feed a small NVIDIA Jetson that runs everything at the edge, so the call happens on the court in under a second with nothing leaving it. I co-founded it and spend most of my time on the modeling and training, while my co-founder does more of the data collection and the physical build. Beyond the model, I built a config-driven, agentic ML workflow on an HPC cluster where training, evaluation, and tracking all run from plain-English specs, with graph-based orchestration that logs every run and survives individual step failures. Backed by the Alpha Fund, Northeastern's advanced-prototype grant.",
